...
def test_provider_config():
    """Test 10: Verify LLM provider configuration."""
    print("\n=== TEST GROUP 10: Provider Config ===")

    provider = CONFIG_DIR / "provider.json"
    test("provider.json exists", provider.is_file())
    if provider.is_file():
        try:
            data = json.loads(provider.read_text())
            test("provider.json is valid JSON", True)
            test("Has default_provider", "default_provider" in data)
            test("Has providers section", "providers" in data and isinstance(data["providers"], dict))

            if "providers" in data:
                for pname, pconfig in data["providers"].items():
                    test(f"Provider '{pname}' has api_key_env", "api_key_env" in pconfig)
                    test(f"Provider '{pname}' has models", "models" in pconfig)
                    if "models" in pconfig:
                        for tier in ["reasoning", "writing", "fast"]:
                            test(f"Provider '{pname}' has '{tier}' tier",
                                 tier in pconfig["models"],
                                 f"Missing tier: {tier}")
        except json.JSONDecodeError as e:
            test("provider.json is valid JSON", False, str(e))

    # Verify agent skills use tier-based model references
    for skill_name in ["writer_agent.md", "scorer_agent.md", "learn_agent.md"]:
        skill_path = SCRIPTS_DIR / skill_name
        if skill_path.is_file():
            content = skill_path.read_text()
            test(f"{skill_name} uses tier-based model", "Tier:" in content,
                 "Should reference 'Tier:' instead of a specific model name")

    # Verify llm_client.py exists
    llm_client = SCRIPTS_DIR / "llm_client.py"
    test("llm_client.py exists", llm_client.is_file())
    if llm_client.is_file():
        content = llm_client.read_text()
        test("llm_client has LLMClient class", "class LLMClient" in content)
        test("llm_client has complete method", "def complete" in content)
